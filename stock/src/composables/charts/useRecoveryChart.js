        ...
        scenarios.forEach(scenario => {
            ...
            if (priceForReinvest > 0) {
                while (recoveredAmount < remainingPrincipal.value) {
                    if (payoutsReinvest > payoutsPerYear.value * 100) { payoutsReinvest = Infinity; break; }
                ...
                results[`${scenario}_reinvest`] = (payoutsReinvest * 12) / payoutsPerYear.value;
            } else {
                results[`${scenario}_reinvest`] = Infinity;
            }
        });
    ...
    const recoveryChartData = computed(() => {
        ...
        return {
            ...
            datasets: [
                {
                    label: '재투자 O (복리)',
                    data: [times.hope_reinvest, times.avg_reinvest, times.despair_reinvest].map(t => t > 0 ? t : 0),
                ...
                {
                    label: '재투자 X (단리)',
                    data: [times.hope_no_reinvest, times.avg_no_reinvest, times.despair_no_reinvest].map(t => t > 0 ? t : 0),
    ...
    const recoveryChartOptions = computed(() => {
        ...
        return {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: (context) => ` ${formatMonthsToYearsForLabel(context.raw)}`,
                    }
                },
                datalabels: {
                    color: '#ffffff',
            ...
            scales: {
                ...
                y: {
                    beginAtZero: true,
                    max: yAxisMax,
                    ticks: { color: textColorSecondary, callback: (value) => `${value.toFixed(0)}개월` },
