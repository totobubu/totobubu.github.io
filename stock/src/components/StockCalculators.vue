...
import CalculatorResult from './calculators/CalculatorResult.vue'; // [핵심] 결과 컴포넌트 import
...
const currentPrice = computed(() => {
    ...
    const latestRecord = props.dividendHistory.find(r => r['당일종가'] && r['당일종가'] !== 'N/A');
    return latestRecord ? parseFloat(latestRecord['당일종가'].replace('$', '')) || 0 : 0;
});
const payoutsPerYear = computed(() => {
    if (!props.dividendHistory || props.dividendHistory.length === 0) return 0;
    const oneYearAgo = new Date(); oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
    const pastYear = props.dividendHistory.filter(i => parseYYMMDD(i['배당락']) >= oneYearAgo);
...
const sharesToBuy = computed(() => !investmentAmount.value || calculatedPrice.value <= 0 ? 0 : investmentAmount.value / calculatedPrice.value);
        ...
        <template #content>
            <Tabs value="0" class="p-tabs-calculator">
                <TabList id="t-calculator-tabs">
                    <Tab value="0"><i class="pi pi-replay mr-2"></i> <span>투자원금 회수 기간 계산기</span></Tab>
                ...
                <TabPanels>
                    <TabPanel value="0">
                        <div class="card" id="t-calculator-step" v-if="deviceType === 'mobile'">
                            <Stepper value="1">
                                <StepItem value="1">
                                    <Step><span>초기 투자금</span><Tag severity="contrast" :value="`$${investmentAmount.toFixed(2)}`"></Tag></Step>
                                    <StepPanel><InputGroup><InputGroupAddon>$</InputGroupAddon><InputNumber v-model="investmentAmount" inputId="investment" mode="currency" currency="USD" locale="en-US" /></InputGroup></StepPanel>
                                </StepItem>
                                <StepItem value="2">
                                    <Step><span>참고 주식 가격</span><span><Tag severity="contrast">${{calculatedPrice.toFixed(2)}} / {{sharesToBuy.toFixed(2)}} 주</Tag></span></Step>
                                ...
                                <StepItem value="4">
                                    <Step><span>배당소득세 15%</span><Tag severity="contrast">{{ applyTax ? '세후' : '세전' }}</Tag></Step>
                                    <StepPanel>
                        ...
                        <Splitter v-else  id="t-calculator-return">
                            <SplitterPanel class="flex items-center justify-center flex-column gap-5 p-4" :size="deviceType === 'tablet' ? '60' : '50'">
                                <div class="flex flex-column gap-2 w-full"><label>초기 투자금</label><InputNumber v-model="investmentAmount" mode="currency" currency="USD" locale="en-US" /></div>
                                <div class="flex flex-column gap-2 w-full"><label>참고 주식 가격</label><SelectButton v-model="priceScenario" :options="priceScenarioOptions" optionLabel="label" optionValue="value" /></div>
                                <div class="flex flex-column gap-2 w-full"><label>지나간 배당금 참고</label><SelectButton v-model="recoveryPeriod" :options="periodOptions" optionLabel="label" optionValue="value" /></div>
                                <div class="flex flex-column gap-2 w-full"><label>배당소득세 15%</label><SelectButton v-model="applyTax" :options="taxOptions" optionValue="value" dataKey="value"><template #option="slotProps"><i :class="slotProps.option.icon" v-tooltip.bottom="slotProps.option.tooltip"></i><span>{{ slotProps.option.tooltip }}</span></template></SelectButton></div>
                            </SplitterPanel>
                            <SplitterPanel class="flex items-center justify-center p-4" :size="deviceType === 'tablet' ? '40' : '50'" :minSize="10">
                                <CalculatorResult 
                                    :formatMonthsToYears="formatMonthsToYears"
                                    :dividendStats="dividendStats"
                                    :recoveryTimes="recoveryTimes"
...
<style scoped>
.chart-container-mobile { position: relative; height: 250px; }
.chart-container-desktop { position: relative; height: 300px; }
</style>