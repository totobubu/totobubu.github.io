...
def fetch_dynamic_ticker_info(ticker_symbol):
    try:
        ...
        yield_val = ((info.get("trailingAnnualDividendRate", 0) / current_price) * 100) if current_price else 0
        earnings_date_str = "N/A"
        if info.get("earningsTimestamp"):
            earnings_date_str = datetime.fromtimestamp(info.get("earningsTimestamp"), tz=timezone.utc).astimezone(timezone(timedelta(hours=9))).strftime("%Y-%m-%d")
...
if __name__ == "__main__":
    ...
    for info_from_nav in all_tickers_info_list:
        ...
        new_info_base = { "Symbol": info_from_nav.get("symbol"), "longName": info_from_nav.get("longName"), "company": info_from_nav.get("company"), "frequency": info_from_nav.get("frequency"), "group": info_from_nav.get("group") }
        ...
        for key, value in final_ticker_info.items():
            ...
            elif key in ["TotalReturn", "payoutRatio"]: final_ticker_info[key] = format_percent(value)
            elif key == "Yield": final_ticker_info[key] = f"{value:.2f}%" if isinstance(value, (int, float)) and value > 0 else "N/A"
        ...
        if new_update_date != old_update_date:
            for key, new_value in final_ticker_info.items():
                ...
                change_status = "equal"
                new_numeric, old_numeric = parse_numeric_value(new_value), parse_numeric_value(old_value)
                if new_numeric is not None and old_numeric is not None:
                    if new_numeric > old_numeric: change_status = "up"
                    elif new_numeric < old_numeric: change_status = "down"
                elif str(new_value) != str(old_value): change_status = "up"
                if change_status != "equal": changes_obj[key] = {"value": old_value, "change": change_status}
        else:
