...
def calculate_yields(history):
    ...
    parsed_history = sorted(
        [
            {"date": datetime.strptime(item["배당락"], "%y. %m. %d"), "data": item}
            for item in history
            if item.get("배당락")
        ],
        key=lambda x: x["date"],
    ...
    for i, current_entry in enumerate(parsed_history):
        ...
        if price_raw and price_raw > 0 and dividend_raw is not None:
            ...
            dividends_in_past_year = sum(
                ...
                and isinstance(past_entry["data"].get("배당금"), (int, float))
            )
            # 배당률은 퍼센트 값이므로 100을 곱하지 않은 순수 비율(0.05 등)로 저장
            yield_rate = dividends_in_past_year / price_raw
            item["배당률"] = yield_rate
...
def process_ticker(item, DESIRED_KEY_ORDER):
    ...
    if not prices_data and not dividends_data and not original_history:
        return False

    historical_prices_map = {p["date"]: p for p in prices_data}
    history_map = {
        h["배당락"]: {
            k: (parse_numeric_value(v) if k != "배당락" else v) for k, v in h.items()
        }
        for h in original_history
        if h.get("배당락")
    }
    ...
    if historical_prices_map:
        for ex_date_str, entry in history_map.items():
            if not any(
                k in entry for k in ["전일종가", "당일시가", "당일종가", "익일종가"]
            ):
                try:
    ...
    if json.dumps(original_history, sort_keys=True) != json.dumps(
        ...
        if save_json_file(file_path, existing_data):
            tqdm.write(f" => UPDATED Dividend History for {ticker}")
            return True
...
def main():
    ...
    if not nav_data:
        return
    print("\n--- Starting Dividend History Update (RAW DATA) ---")
    active_tickers_info = [
