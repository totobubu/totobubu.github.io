...
def parse_date(date_str):
    ...
    if date_obj.weekday() == 5:  # 토요일
        ...
        print(
            f"[INFO] 토요일 날짜 감지, 금요일({parsed_date})로 조정 (원래: {original_date})"
        )
    elif date_obj.weekday() == 6:  # 일요일
        date_obj = date_obj - timedelta(days=2)
        parsed_date = date_obj.strftime("%Y-%m-%d")
        print(
            f"[INFO] 일요일 날짜 감지, 금요일({parsed_date})로 조정 (원래: {original_date})"
        )
...
def classify_asset(security_name, identifier):
    ...
    # 옵션 계약
    if (
        re.search(r"\d{6}[CP]\d{8}", id_upper)
        or re.search(r" C\d+", name_upper)
        or re.search(r" P\d+", name_upper)
        or "CALL" in name_upper
        or "PUT" in name_upper
    ):
        return "option"
...
def parse_holdings_data(data_text):
    ...
    has_header = any(
        keyword in first_line for keyword in ["ticker", "name", "weight", "identifier"]
    )
    ...
    for line in data_lines:
        ...
        try:
            ...
            if market_value_text:
                market_value_clean = (
                    market_value_text.replace(",", "").replace("$", "").strip()
                )
                try:
...
def get_ticker_market(ticker):
    ...
    try:
        ...
        if not nav_path.exists():
            print(f"[WARNING] nav.json 파일을 찾을 수 없습니다: {nav_path}")
            return None
        ...
        print(f"[WARNING] {ticker}: nav.json에서 market 정보를 찾을 수 없습니다")
        return None
    except Exception as e:
...
def find_json_file_path(ticker):
    """
    ...
    market_dir = get_ticker_market(ticker)
    
    if market_dir:
        # market 디렉토리에서 찾기
        json_path = Path("public/data") / market_dir / f"{sanitized}.json"
        if json_path.exists():
            return json_path
    ...
    if data_dir.exists():
        for market_subdir in data_dir.iterdir():
            if market_subdir.is_dir():
                json_path = market_subdir / f"{sanitized}.json"
                if json_path.exists():
                    print(f"[INFO] {ticker}: {json_path} 발견 (자동 검색)")
                    return json_path
...
def update_json_file(ticker, date, holdings):
    ...
    """
    # 파일 경로 찾기
    json_path = find_json_file_path(ticker)

    if not json_path:
        print(f"[ERROR] {ticker}: JSON 파일을 찾을 수 없습니다")
        print(f"  찾은 위치: public/data/{{market}}/{ticker.lower().replace('.', '-')}.json")
        return False
    ...
    for section in sections:
        ...
        for i, line in enumerate(lines):
            ...
            if (
                not ticker
                and line_stripped
                and re.match(r"^[A-Z]{2,6}$", line_stripped)
            ):
...
def main():
    ...
    if len(sys.argv) > 3 and sys.argv[3] != "--stdin":
        ...
        # 표준 입력에서 읽기
        print(
            "데이터를 입력하세요 (Ctrl+Z then Enter to finish on Windows, Ctrl+D on Unix):"
        )
    ...
    try:
        ...
        print("-" * 100)
        print(
            f"{'No':<4} {'Name':<40} {'Symbol':<10} {'Weight':>8} {'Shares':>12} {'Market Value':>15} {'Type':<12}"
        )
        print("-" * 100)
        for i, h in enumerate(holdings, 1):
            ...
            print(
                f"{i:<4} {name_short:<40} {h['symbol']:<10} {h['weight']:7.2f}% {shares_str:>12} {mv_str:>15} {h['type']:<12}"
            )
        print("-" * 100)
