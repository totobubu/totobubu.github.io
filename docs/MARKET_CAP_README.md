...
**동작 원리:**
```
marketCap (근사) = sharesOutstanding (현재) × close (과거)
```
...
**주의사항:**
- ⚠️ **근사값**: 정확도 85~95%
- ⚠️ **가정**: 발행주식수가 변하지 않았다고 가정
- ⚠️ **오차 발생 가능**: 주식분할, 증자, 자사주 매입이 있었다면
- ✅ **이미 있는 값 보존**: 기존 marketCap은 덮어쓰지 않음
