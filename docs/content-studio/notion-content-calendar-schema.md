...
| Provider | Select | 운용사 |
| Ticker | Text | 티커 |
| Status | Select | Draft, Ready, Published, Hold |
...
통합 연결에는 해당 데이터 소스를 공유하고 `NOTION_TOKEN`, `NOTION_DATA_SOURCE_ID` 환경 변수를 설정한다. 토큰은 `.env`나 저장소에 커밋하지 않는다.

동기화는 `Event Key`로 기존 페이지를 찾아 생성 또는 갱신하므로 반복 실행해도 같은 이벤트가 중복 생성되지 않는다.
