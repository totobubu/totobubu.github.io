...
- `events-report.json` : 감지 결과 전체 로그 (세그먼트, 배당 간격, confidence 등)
- `events-config-template.json` : 적용 후보 템플릿 (`apply: false` 기본)

## 2. 검토 & 승인

1. `scripts/output/events-report.json`으로 근거(주기, 평균 일수, confidence)를 확인합니다.
...
향후 요일 변경 감지 등 세부 이벤트를 추가할 때는 `events` 아래에 새로운 배열을 확장하면 됩니다.
