...
2. 데이터 변경이 없고, 마지막 업데이트가 3시간 이내 → Update 필드 유지 (파일 저장 안함)
3. 데이터 변경이 없고, 마지막 업데이트가 3시간 초과 → Update 필드 갱신
```
...
| 3시간마다 실행 (데이터 변경 없음) | 8회/일 | 8회/일 | 0% |
...
(다음날 변경 없음 - 커밋 없음)
ℹ️ Auto: Update info data (daily)  # 3시간 후 또는 변경 있을 때
```
...
✅ **데이터 변경 없고 3시간 이내** → Update 유지 + 파일 저장 안함  
✅ **데이터 변경 없고 3시간 초과** → Update 갱신 + 파일 저장  
