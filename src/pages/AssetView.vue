...
const loadMemberData = async (memberId) => {
    ...
    allAssets.forEach((asset) => {
        ...
        Object.entries(asset.holdings).forEach(([accountId, holding]) => {
            if (accountIds.has(accountId) && holding.amount > 0) {
...
<template>
    <div id="t-asset-view">
            ...
            <template #content>
                <div class="p-3">
                    ...
                    <div class="flex gap-2 mb-2">
                        <Button label="데이터 마이그레이션 (구조 변경)" severity="warning" size="small" @click="migrateData" />
                    </div>
        ...
        <Card v-if="familyMembers.length === 0 && !isLoadingMembers">
            <template #content>
                <div class="flex flex-column align-items-center justify-content-center p-8 gap-4">
                    ...
                    <Button label="첫 번째 가족 추가하기" icon="pi pi-plus" size="large" @click="openAddMemberDialog" />
                </div>
            </template>
        </Card>

        <!-- 가족 멤버 리스트 -->
        <FamilyMemberList v-if="familyMembers.length > 0" :members="familyMembers" :selectedIndex="selectedTabIndex"
        ...
        <div v-if="selectedMember" class="mt-3">
            ...
            <div class="flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                ...
                <div class="flex align-items-center gap-2">
                    <InputSwitch v-model="showZeroBalanceAssets" inputId="showZeroBalance" />
                    <label for="showZeroBalance" class="cursor-pointer select-none">보유 수량 0인 항목 보기</label>
                </div>
            </div>

            <!-- 증권사 추가 버튼 (제거 또는 계좌 추가로 변경) -->
            <div class="flex justify-content-end gap-2 mb-3">
                <Button label="거래내역서 업로드" icon="pi pi-upload" severity="success" @click="openBrokerageUploadDialog" />
                <Button label="계좌 직접 추가" icon="pi pi-plus" @click="openAddAccountDialog(selectedMember.id)" />
            </div>
            ...
            <Card v-if="viewMode === 'account'">
                ...
                <template #content>
                    <TreeTable :value="treeDataMap[selectedMember.id]" :metaKeySelection="false" selectionMode="single"
                        ...
                        <Column field="name" header="자산" expander>
                            <template #body="{ node }">
                                <div class="flex align-items-center gap-2">
                                    ...
                                    <Tag v-if="node.data.type" :value="node.data.type" :severity="node.data.type === '증권사'
                                        ? 'success'
                                        : node.data.type === '계좌'
                                            ? 'warning'
                                            : node.data.type ===
                        ...
                        <Column header="액션" style="width: 200px">
                            <template #body="{ node }">
                                <div class="flex gap-2">
                                    <Button icon="pi pi-upload" size="small" rounded severity="info"
                                        v-tooltip="'파일 업로드'" @click="openUploadDialog(node)" />
                                    <Button icon="pi pi-plus" size="small" rounded v-tooltip="'추가'"
                                        @click="addChild(node)" />
                                    <Button icon="pi pi-pencil" size="small" rounded severity="secondary"
                                        v-tooltip="'수정'" @click="editNode(node)" />
                                    <Button icon="pi pi-trash" size="small" rounded severity="danger" v-tooltip="'삭제'"
                    ...
                    ">
                        <template #content>
                            <div class="flex flex-column align-items-center gap-3 p-4">
                                ...
                                <p>등록된 자산이 없습니다.</p>
                                <Button label="첫 번째 계좌 추가하기" icon="pi pi-plus" @click="
            ...
            <div v-if="viewMode === 'asset_type'" class="mt-4">
                <AssetTypeDataTable :assets="assetTypeDataMap[selectedMember.id] || []" />
            </div>

            <!-- 종목 기준 보기 (별도 패널) -->
            <div v-if="viewMode === 'stock'" class="mt-4">
                <AssetListView :viewMode="viewMode" :treeData="treeDataMap[selectedMember.id] || []" />
            </div>
        </div>
            ...
            <Card>
                <template #content>
                    <div class="flex flex-column align-items-center justify-content-center p-8 gap-4">
                        ...
                        <Button label="가족 추가하기" icon="pi pi-plus" size="large" @click="openAddMemberDialog" />
                    </div>
                </template>
            </Card>
        </div>
        ...
        <!-- 가족 멤버 다이얼로그 -->
        <Dialog v-model:visible="showMemberDialog" modal :style="{ width: '30rem' }">
            <template #header>
            ...
            <div class="flex flex-column gap-3">
                <div class="flex flex-column gap-2">
                    ...
                    <InputText id="memberName" v-model="memberForm.name" placeholder="이름을 입력하세요" />
                </div>
                <div class="flex flex-column gap-2">
                    <label for="memberRelationship">관계</label>
                    <Dropdown id="memberRelationship" v-model="memberForm.relationship" :options="relationships"
                        optionLabel="label" optionValue="value" placeholder="관계를 선택하세요" />
                </div>
            </div>
            <template #footer>
                <Button label="취소" severity="secondary" @click="showMemberDialog = false" />
                <Button label="저장" @click="saveMember" />
            </template>
        </Dialog>

        <!-- 증권사 다이얼로그 -->
        <Dialog v-model:visible="showBrokerageDialog" modal :style="{ width: '30rem' }">
            <template #header>
            ...
            <div class="flex flex-column gap-3">
                <div class="flex flex-column gap-2">
                    ...
                    <InputText id="brokerageName" v-model="brokerageForm.name" placeholder="예: kb자산운용, 미래에셋" />
                </div>
            </div>
            <template #footer>
                <Button label="취소" severity="secondary" @click="showBrokerageDialog = false" />
                <Button label="저장" @click="saveBrokerage" />
            </template>
        </Dialog>

        <!-- 계좌 다이얼로그 -->
        <Dialog v-model:visible="showAccountDialog" modal :style="{ width: '30rem' }">
            <template #header>
            ...
            <div class="flex flex-column gap-3">
                <div class="flex flex-column gap-2">
                    ...
                    <InputText id="accountName" v-model="accountForm.name" placeholder="계좌명을 입력하세요" />
                </div>
                <div class="flex flex-column gap-2">
                    <label for="accountNumber">계좌번호 (선택)</label>
                    <InputText id="accountNumber" v-model="accountForm.accountNumber" placeholder="계좌번호를 입력하세요" />
                </div>
            </div>
            <template #footer>
                <Button label="취소" severity="secondary" @click="showAccountDialog = false" />
                <Button label="저장" @click="saveAccount" />
            </template>
        </Dialog>

        <!-- 자산 다이얼로그 -->
        <Dialog v-model:visible="showAssetDialog" modal :style="{ width: '30rem' }">
            <template #header>
            ...
            <div class="flex flex-column gap-3">
                <div class="flex flex-column gap-2">
                    ...
                    <Dropdown id="assetType" v-model="assetForm.type" :options="assetTypes" optionLabel="label"
                        optionValue="value" placeholder="자산 종류를 선택하세요" />
                </div>
                <div v-if="assetForm.type === '주식'" class="flex flex-column gap-2">
                    <label for="assetSymbol">심볼</label>
                    <InputText id="assetSymbol" v-model="assetForm.symbol" placeholder="예: AAPL, TSLA" />
                </div>
                <div class="flex flex-column gap-2">
                    <label for="assetAmount">수량/금액</label>
                    <InputNumber id="assetAmount" v-model="assetForm.amount" :min="0" placeholder="수량 또는 금액" />
                </div>
                <div class="flex flex-column gap-2">
                    <label for="assetCurrency">통화</label>
                    <Dropdown id="assetCurrency" v-model="assetForm.currency" :options="currencies" optionLabel="label"
                        optionValue="value" placeholder="통화를 선택하세요" />
                </div>
                <div class="flex flex-column gap-2">
                    <label for="assetNotes">메모 (선택)</label>
                    <Textarea id="assetNotes" v-model="assetForm.notes" rows="3" placeholder="추가 정보를 입력하세요" />
                </div>
            </div>
            <template #footer>
                <Button label="취소" severity="secondary" @click="showAssetDialog = false" />
                <Button label="저장" @click="saveAsset" />
            </template>
        </Dialog>

        <!-- 파일 업로드 다이얼로그 -->
        <Dialog v-model:visible="showUploadDialog" modal :style="{ width: '40rem' }" header="파일 업로드">
            <div class="flex flex-column gap-4">
                ...
                <FileUpload mode="basic" name="upload[]" accept=".pdf,.xlsx,.xls,.csv" :maxFileSize="5000000"
                    @upload="handleFileUpload" :auto="false" chooseLabel="파일 선택" class="w-full">
                    <template #empty>
                        <p class="text-center">
            ...
            <template #footer>
                <Button label="취소" severity="secondary" @click="showUploadDialog = false" />
            </template>
        </Dialog>
        <!-- Add Asset Modal -->
        <AddAssetModal v-model:visible="showAddAssetModal" :showAssetType="true" @saved="handleAssetSaved" />

        <!-- 거래내역서 업로드 다이얼로그 -->
        <BrokerageUploadDialog v-if="selectedMember" v-model:visible="showBrokerageUploadDialog"
            :memberId="selectedMember.id" @upload-complete="handleTransactionUploadComplete" />

        <!-- 종목명 매핑 다이얼로그 -->
        <StockMappingDialog v-if="uploadedTransactionData" v-model:visible="showStockMappingDialog"
            :transactions="uploadedTransactionData.extractedData.transactions" :brokerage="uploadBrokerage"
