			...
			promises.push(
				fetch(`/nav/${market}/${char}.json`)
					...
					.then(data => {
						data.forEach(item => {
							if (item.symbol) {
								localData.set(`${market}_${item.symbol}`, item);
							}
						});
					})
					.catch(() => {
	...
	try {
		...
		querySnapshot.forEach((doc) => {
			...
			if (localItem) {
				...
				const isSymbolMatch = localItem.symbol === mapping.systemTicker;
				const isIsinMatch = !mapping.brokerageTicker || localItem.isin === mapping.brokerageTicker; // If mapping has no ISIN, ignore ISIN check? Or strict? User said "symbol, isin, koName"
				// Let's be strict if mapping has ISIN.
				const isKoNameMatch = localItem.koName === mapping.brokerageStockName; // brokerageStockName is the koName from the approval
			...
			<template #header>
				<div class="flex justify-content-between align-items-center p-3">
					...
					<div class="flex gap-2">
						<Button label="매핑 데이터 내보내기" icon="pi pi-download" severity="secondary"
							@click="exportMappings" />
						<Button label="매핑 동기화" icon="pi pi-sync" severity="help" @click="handleSync" />
						<Button label="새로고침" icon="pi pi-refresh" @click="loadPendingApprovals" />
					</div>
				</div>
			</template>
			<template #content>
				...
				<Message v-else-if="pendingApprovals.length === 0" severity="info" :closable="false">
					승인 대기 중인 koName이 없습니다.
				</Message>

				<DataTable v-else :value="pendingApprovals" :paginator="true" :rows="20"
					...
					<Column field="symbol" header="SYMBOL" sortable>
						<template #body="{ data }">
							<InputText v-model="data.symbol" class="p-inputtext-sm w-full" placeholder="티커 입력" />
						</template>
					</Column>
					...
					<Column header="액션" style="width: 200px">
						<template #body="{ data }">
							<div class="flex gap-2">
								...
								<Button label="거부" icon="pi pi-times" severity="danger" size="small"
