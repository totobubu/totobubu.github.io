...
<template>
	<Drawer v-model:visible="isVisible" :header="title" position="full">
		...
		<div v-else-if="displayTransactions.length === 0" class="text-center p-4">
		...
		<div v-else class="asset-transaction-container">
			...
			<div class="filter-section p-3 bg-gray-50 border-bottom-1">
				<div class="flex gap-3 align-items-center">
					...
					<div class="flex flex-column">
						<label for="year-filter" class="text-sm mb-1">년도</label>
						<Calendar v-model="selectedYear" view="year" dateFormat="yy" placeholder="년도 선택"
							:showIcon="true" style="width: 150px" />
					</div>

					<!-- 거래타입 필터 -->
					<div class="flex flex-column">
						<label for="type-filter" class="text-sm mb-1">거래타입</label>
						<Select v-model="selectedTransactionType" :options="transactionTypes" optionLabel="label"
							optionValue="value" placeholder="타입 선택" style="width: 200px" />
					</div>

					<!-- 필터 초기화 버튼 -->
					<Button label="필터 초기화" icon="pi pi-filter-slash" size="small" text @click="resetFilters"
						class="mt-4" />
			...
			<DataTable :value="filteredTransactions" class="asset-transaction-table" :scrollable="true"
				scrollHeight="calc(100vh - 260px)" scrollDirection="both" stripedRows>
				<!-- 날짜 (Frozen) -->
				<Column field="날짜" header="날짜" :frozen="true" :style="{ minWidth: '120px', width: '120px' }">
				...
				<!-- 평단 -->
				<Column v-if="shouldShowColumn('평단')" field="평단" header="평단" :style="{ minWidth: '100px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 보유량 -->
				<Column v-if="shouldShowColumn('보유량')" field="보유량" header="보유량" :style="{ minWidth: '100px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 거래(수량) -->
				<Column v-if="shouldShowColumn('거래(수량)')" field="거래(수량)" header="거래(수량)" :style="{ minWidth: '110px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 거래(달러) -->
				<Column v-if="shouldShowColumn('거래(달러)')" field="거래(달러)" header="거래(달러)" :style="{ minWidth: '110px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 거래(원화) -->
				<Column v-if="shouldShowColumn('거래(원화)')" field="거래(원화)" header="거래(원화)" :style="{ minWidth: '110px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 배당락 -->
				<Column v-if="shouldShowColumn('배당락')" field="배당락" header="배당락" :style="{ minWidth: '160px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 배당기준 -->
				<Column v-if="shouldShowColumn('배당기준')" field="배당기준" header="배당기준" :style="{ minWidth: '100px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 세후배당금($) -->
				<Column v-if="shouldShowColumn('세후배당금($)')" field="세후배당금($)" header="세후배당금($)"
				...
				<!-- 외화과표 -->
				<Column v-if="shouldShowColumn('외화과표')" field="외화과표" header="외화과표" :style="{ minWidth: '110px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 원화과표 -->
				<Column v-if="shouldShowColumn('원화과표')" field="원화과표" header="원화과표" :style="{ minWidth: '130px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 세후배당금(원화) -->
				<Column v-if="shouldShowColumn('세후배당금(원화)')" field="세후배당금(원화)" header="세후배당금(원화)"
				...
				<!-- 환율 -->
				<Column v-if="shouldShowColumn('환율')" field="환율" header="환율" :style="{ minWidth: '100px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 지급기준 -->
				<Column v-if="shouldShowColumn('지급기준')" field="지급기준" header="지급기준" :style="{ minWidth: '100px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 원금회수율 -->
				<Column v-if="shouldShowColumn('원금회수율')" field="원금회수율" header="원금회수율" :style="{ minWidth: '110px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 누적배당 -->
				<Column v-if="shouldShowColumn('누적배당')" field="누적배당" header="누적배당" :style="{ minWidth: '110px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 누적TR -->
				<Column v-if="shouldShowColumn('누적TR')" field="누적TR" header="누적TR" :style="{ minWidth: '110px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 당일종가 -->
				<Column field="당일종가" header="당일종가" :style="{ minWidth: '100px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 당일 평단 -->
				<Column field="당일 평단" header="당일 평단" :style="{ minWidth: '100px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
				...
				<!-- 당일평가액 -->
				<Column field="당일평가액" header="당일평가액" :style="{ minWidth: '120px' }">
					<template #body="slotProps">
						<div :class="{
							'summary-row': isSummaryRow(slotProps.data),
							'dividend-row': hasDividend(slotProps.data),
