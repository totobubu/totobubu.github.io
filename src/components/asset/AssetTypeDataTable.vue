...
<template>
    <div class="flex flex-column gap-4">
        <DataTable :value="assets" stripedRows class="p-datatable-sm shadow-1" rowGroupMode="subheader"
            groupRowsBy="groupName" sortMode="single" sortField="groupName" :sortOrder="1">
            <template #groupheader="slotProps">
                <div class="flex align-items-center gap-2 font-bold text-lg p-3 surface-100">
                    <i :class="getGroupIcon(slotProps.data.groupKey)"></i>
                    <span>{{ slotProps.data.groupName }}</span>
                    <Tag :value="getGroupSummary(slotProps.data.groupKey)" severity="info" class="ml-2" />
                </div>
            </template>
            ...
            <Column field="profitRate" header="총 수익률" sortable>
                <template #body="slotProps">
                    <span :class="getProfitColorClass(slotProps.data.profitRate)">
            ...
            <Column field="profitAmount" header="총 수익금" sortable>
                <template #body="slotProps">
                    <span :class="getProfitColorClass(slotProps.data.profitAmount)
                        ">
                        {{
                            formatCurrency(
            ...
            <Column header="내역" style="width: 80px">
                <template #body="slotProps">
                    <Button icon="pi pi-list" text rounded severity="secondary" @click="openHistory(slotProps.data)"
                        v-tooltip="'거래 내역'" />
                </template>
            </Column>
        </DataTable>

        <TransactionHistoryDialog v-if="selectedAsset" v-model:visible="showHistoryDialog"
            :accountId="selectedAsset.accountId" :assetId="selectedAsset.id" />
    </div>
</template>
