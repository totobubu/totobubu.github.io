                            ...
                            <div class="flex flex-column gap-3">
                                ...
                                " class="flex flex-column gap-2">
                                    <div class="text-sm font-semibold text-color-secondary">
                                        검색 결과 ({{ stock.searchResults.length }}개):
                                    </div>
                                    <div class="flex flex-column gap-2">
                                        <div v-for="result in stock.searchResults.slice(0, 5)" :key="result.symbol"
                                            ...
                                            <div class="flex justify-content-between align-items-center">
                                                <div>
                                                    <div class="font-semibold" :class="{
                                                        'text-white': stock.selectedTicker === result.symbol
                                                    }">
                                                        {{ result.symbol }}
                                                    </div>
                                                    <div class="text-sm" :class="{
                                                        'text-white': stock.selectedTicker === result.symbol,
                                                        'text-color-secondary': stock.selectedTicker !== result.symbol
                                                    }">
                                                        {{ result.name }}
                                                    </div>
                                                </div>
                                                <i v-if="stock.selectedTicker === result.symbol"
        ...
        if (mapping) {
            ...
            try {
                ...
                if (results && results.length > 0) {
                    ...
                    // ISIN 형식의 심볼 필터링 (.SG, .KS 등이 붙은 것)
                    const pureSymbols = results.filter(r => !r.symbol.includes('.'));
                    const isinSymbols = results.filter(r => r.symbol.includes('.'));
                    ...
                    // 순수 심볼이 1개이고 나머지가 ISIN 형식이면, 순수 심볼 자동 선택
                    else if (pureSymbols.length === 1 && isinSymbols.length > 0) {
                        selectTicker(stock, pureSymbols[0]);
                        stock.isAutoMatched = true;
                    }
                    else {
