...
<template>
	<Dialog v-model:visible="isVisible" modal header="종목명 매핑" :style="{ width: '900px', maxHeight: '90vh' }"
		...
		<div class="flex flex-column gap-4">
			...
			<div v-if="manualMappingStocks.length > 0">
				...
				<div class="flex flex-column gap-3" style="max-height: 40vh; overflow-y: auto">
					<Card v-for="stock in manualMappingStocks" :key="stock.stock_name" class="shadow-1">
						<template #content>
							<div class="flex flex-column gap-3">
								...
								<div class="flex justify-content-between align-items-center gap-3">
									<div class="flex align-items-center gap-3 flex-1">
										...
										<div class="text-sm text-color-secondary flex gap-3">
									...
									<Tag v-if="stock.mappedTicker" value="매핑 완료" severity="success"
										icon="pi pi-check" />
									<Button v-else icon="pi pi-link" label="티커 매핑" size="small"
										:disabled="!stock.selectedTicker" @click="mapStock(stock)" />
								</div>
								...
								" class="flex flex-column gap-2">
									...
									<div class="flex flex-column gap-2">
										<div v-for="result in stock.searchResults.slice(
											0,
											5
										)" :key="result.symbol" @click="selectTicker(stock, result)"
			...
			<div v-if="autoMatchedStocks.length > 0" class="mt-4">
				...
				<DataTable :value="autoMatchedStocks" stripedRows class="p-datatable-sm shadow-1" scrollable
					scrollHeight="50vh">
		...
		<template #footer>
			<div class="flex justify-content-between">
				<Button label="나중에 하기" severity="secondary" @click="skipMapping" :disabled="isProcessing" />
				<Button label="완료" @click="completeMapping" :loading="isProcessing" />
			</div>
		</template>
	</Dialog>
</template>
...
const extractUniqueStocks = async () => {
	...
	console.log('수량이 있는 거래:', props.transactions.filter(t => t.quantity && t.quantity > 0).length);
	console.log('수량이 없거나 0인 거래:', props.transactions.filter(t => !t.quantity || t.quantity <= 0).length);
	...
	props.transactions.forEach((t) => {
		...
		// 디버깅: 그룹화 상세
		console.log(`Transaction: ${t.stock_name}, Ticker: ${t.ticker}, Key: ${key}`);
	...
	console.log('자동 매핑된 종목:', stocks.filter(s => s.isAutoMatched).length);
	console.log('수동 매핑 필요:', stocks.filter(s => !s.isAutoMatched).length);
	console.log('이미 매핑됨:', stocks.filter(s => s.mappedTicker).length);
	console.log('상세:', stocks.map(s => ({
		name: s.stock_name,
