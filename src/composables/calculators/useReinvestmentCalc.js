...
export function useReinvestmentCalc(dependencies) {
    const { currentAssets, dividendStats, payoutsPerYear, applyTax, currentPrice, isUSD, currency, chartTheme, userBookmark } = dependencies;

    const targetAsset = ref(userBookmark.value?.targetAsset || (isUSD.value ? 100000 : 100000000));
    ...
    const { goalAchievementTimes, chartOptions } = useReinvestmentChart({
        currentAssets, targetAmount: targetAsset, payoutsPerYear, dividendStats,
        annualGrowthRateScenario: growthRateForCalc, applyTax, currentPrice, currency, theme: chartTheme,
    });
