    ...
    try {
        const resultsPromises = symbolArray.map(async (symbol) => {
            try {
                ...
                if (data.chart.error) {
                    throw new Error(data.chart.error.description || 'Unknown error from Yahoo API');
                }

                const result = data.chart.result[0];
                if (!result || !result.timestamp) {
                    return { symbol, prices: [], dividends: [], splits: [], firstTradeDate: null };
                }
                ...
                const firstTradeDate = result.meta.firstTradeTime ? formatDate(result.meta.firstTradeTime) : null;
                
                return { symbol, firstTradeDate, prices, dividends, splits };

            } catch (e) {
                console.error(`[API] Error fetching data for ${symbol}:`, e.message);
                return { symbol, error: e.message, prices: [], dividends: [], splits: [] };
            }
        });

        const exchangeRatesPromise = axios.get(`https://query1.finance.yahoo.com/v8/finance/chart/USDKRW=X?period1=${period1}&period2=${period2}&interval=1d`, { headers: YF_HEADERS })
            .then(({ data }) => {
                const result = data.chart.result[0];
        ...
        return res.status(200).json({ tickerData, exchangeRates: exchangeRatesData });
